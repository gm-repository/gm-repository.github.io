...
categories: [DevOps, Automação, Ansible]
...
- name: Configuração com Filtros
  ...
  tasks:
    - name: Exibir Nome de Usuário em Maiúsculas
      debug:
        msg: "{% raw %}{{ user_name | upper }}{% endraw %}"

    - name: Mensagem de Servidor com Valor Padrão
      debug:
        msg: "{% raw %}{{ server_message | default('Servidor em Manutenção') }}{% endraw %}"
```
...
- name: Configuração do Servidor Web com Variáveis e Filtros
  ...
  vars:
    apache_package: apache2
    welcome_message: "Bem-vindo ao {% raw %}{{ environment | default('desenvolvimento') }}{% endraw %}"
  ...
  tasks:
    ...
    - name: Criar Página Personalizada
      copy:
        content: "{% raw %}{{ welcome_message | upper }}{% endraw %}"
        dest: /var/www/html/index.html
        mode: '0644'
```
