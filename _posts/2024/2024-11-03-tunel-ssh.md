...
> Verifique se o Netcat (`nc`) está instalado no sistema, pois ele é necessário para alguns comandos de encadeamento. Caso não esteja disponível, pode ser instalado em sistemas Debian/Ubuntu com `sudo apt install netcat`{:.command} ou em sistemas CentOS/RHEL com `sudo yum install nc`{:.command}.
{:.prompt-tip}
